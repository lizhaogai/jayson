        ...
        utils.JSON.parse(data, {}, function (err, response) {
            if (err) {
                if (self.callbacks[response.id]) {
                    self.callbacks[response.id](err);
...
MqttClient.prototype._request = function (request, callback) {
    ...
    utils.JSON.stringify(request, options, function (err, body) {
        if (err) {
            if (this.callbacks[request.id]) {
                delete this.callbacks[request.id];
            }
            return callback(err);
        }
        var topic;
        if (self.channel.deviceId && self.channel.channelId) {
            topic = util.format(self._topic, self.channel.deviceId, self.channel.channelId);
        } else if (self.channel.deviceId) {
            topic = util.format(self._topic, self.channel.deviceId);
        } else if (self.channel.channelId) {
            topic = util.format(self._topic, self.channel.channelId);
        } else {
            topic = self.topic;
        }
